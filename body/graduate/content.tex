...
对齐first\_path：在没有通过first\_path对齐的情况下，CIR的起始点是随机的，如图\ref{fig:first_path}所示，其中蓝线被略微抬高，以便与橙线做出区分。

CIR的最大值代表的是没有经过任何反射的直射信号，对应的距离0，而first\_path就是这个0距离点相对于第一个采样点的位置的小数部分，如图\ref{fig:first_path}所示，不同样本同一采样点对应的距离在对齐前是不同的。

图中蓝线的first\_path为0.25ns，橙线的first\_path为0.5ns，为了对齐蓝线与橙线，本文舍弃蓝线前0.25ns的数据，舍弃橙线前0.5ns的数据，从而实现对齐。

由于不同的CIR的first\_path不同，它们舍弃的数据长度不同，在64倍采样的情况下，舍弃的数据长度在0到63之间波动，我们会保留(CIR\_LEN-1)*64个对齐好的采样点，这时每个样本同一采样点对应的距离相同。
